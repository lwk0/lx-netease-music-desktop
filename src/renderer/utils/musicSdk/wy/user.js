...
import { setWyVipType } from '@renderer/store/user/action'
...
export default {
  ...
  async getSublist(limit = 100, offset = 0, retryNum = 0) {
    ...
    try {
      const cookie = appSetting['common.wy_cookie']
      if (!cookie) throw new Error('请先设置网易云 Cookie')
      const csrfToken = (cookie.match(/_csrf=([^(;|$)]+)/) || [])[1]
      ...
      // /weapi/artist/sublist 返回的 body.data 直接就是歌手数组（与手机魔改版一致），
      // 但部分版本可能嵌套在 data.data，这里做兼容处理。
      const raw = body.data
  ...
  async getSubAlbumList(limit = 100, offset = 0, retryNum = 0) {
    ...
    try {
      ...
      // /weapi/album/sublist 返回的 body.data 直接就是专辑数组（与手机魔改版一致），
      // 但部分版本可能嵌套在 data.data，这里做兼容处理。
      const raw = body.data
      const list = Array.isArray(raw) ? raw : (raw?.data || [])
      return {
  ...
  async getAllSublist() {
    ...
    const limit = 100
    while (true) {
      const result = await this.getSublist(limit, offset)
  ...
  async getAllSubAlbumList() {
    ...
    const limit = 100
    while (true) {
      const result = await this.getSubAlbumList(limit, offset)
  ...
  // 二维码登录相关接口
  async getQrKey(retryNum = 0) {
    const maxRetries = 3
    const retryDelay = 200
    try {
      const { body, statusCode } = await httpFetch('https://music.163.com/weapi/login/qr/key', {
        method: 'post',
        headers: {
          'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/108.0.0.0 Safari/537.36 Edg/108.0.1462.54',
          origin: 'https://music.163.com',
      ...
      if (statusCode !== 200 || body.code !== 200) throw new Error(body.message || '获取二维码密钥失败')
      return body.unikey || body.data?.unikey
    } catch (error) {
      if (retryNum < maxRetries) {
        await new Promise(resolve => setTimeout(resolve, retryDelay))
        return this.getQrKey(retryNum + 1)
      }
      throw error
    }
  },

  async createQr(key, retryNum = 0) {
    const maxRetries = 3
    const retryDelay = 200
    try {
      const { body, statusCode } = await httpFetch('https://music.163.com/weapi/login/qr/create', {
        method: 'post',
        headers: {
          'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/108.0.0.0 Safari/537.36 Edg/108.0.1462.54',
          origin: 'https://music.163.com',
        ...
        form: weapi({ key, type: 1, qrimg: true }),
      }).promise
      if (statusCode !== 200 || body.code !== 200) throw new Error(body.message || '创建二维码失败')
      return {
        qrimg: body.data?.qrimg || body.qrimg,
        qrurl: body.data?.qrurl || body.qrurl,
      }
    } catch (error) {
      if (retryNum < maxRetries) {
        await new Promise(resolve => setTimeout(resolve, retryDelay))
        return this.createQr(key, retryNum + 1)
      }
      throw error
    }
  },

  async checkQr(key, retryNum = 0) {
    const maxRetries = 3
    const retryDelay = 200
    try {
      const { body, statusCode, headers } = await httpFetch('https://music.163.com/weapi/login/qr/check', {
        method: 'post',
        headers: {
          'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/108.0.0.0 Safari/537.36 Edg/108.0.1462.54',
          origin: 'https://music.163.com',
      ...
      return {
        code: statusCode === 200 && body.code === 803 ? 803 : body.code,
        message: body.message || '',
        cookie: body.cookie || cookies,
        body,
      }
    } catch (error) {
      if (retryNum < maxRetries) {
        await new Promise(resolve => setTimeout(resolve, retryDelay))
        return this.checkQr(key, retryNum + 1)
      }
      throw error
    }
  },

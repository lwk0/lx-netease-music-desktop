...
## [1.0.2](https://github.com/lwk0/lx-netease-music-desktop/compare/v1.0.1...v1.0.2) - 2026-09-20
...
## [1.0.1](https://github.com/lwk0/lx-netease-music-desktop/compare/v2.12.2...v1.0.1) - 2026-09-05

### 新增

- 新增「设置 → 基本设置 → 开机自动启动」开关，开启后 Windows 登录时自动运行软件（默认最小化到系统托盘启动）
- 新增 APPX / MSIX 版本的更新引导：检测到新版本后可后台下载新版 APPX 安装包到「下载」文件夹，并自动打开安装包，点击安装即可完成覆盖升级（需已信任签名证书，可保留歌单与设置、无需卸载旧版）
- 网易云账号增强：Cookie 登录、红心同步到「我喜欢的音乐」、播放历史同步、在线歌单增删歌曲、每日推荐、评论读写、歌手与专辑详情页

### 优化

- 播放详情页封面与歌词区域由 40/60 调整为 50/50 平分，并增大封面尺寸
- 点击播放详情页封面可切换「原版封面 / 黑胶唱片」效果，黑胶模式下歌曲信息居中显示
- 我的列表新增列表搜索与歌曲内联搜索，歌曲列表工具栏改为 Win11 资源管理器风格
- 统一自定义滚动条样式，与项目 UI 风格保持一致
- 优化封面图加载：主进程为图片请求补充 UA 与 Referer，降低封面加载失败率

### 修复

- 修复网易云网页登录中手机号登录无法加载、QQ 登录弹窗缓慢的问题
- 修复部分歌曲在封面模式下仍显示序号的问题，改为显示占位封面
- 修复软件更新设置页「打开更新窗口」按钮在检查更新时消失的问题
- 修复 dev 模式启动失败（Session 未就绪导致封面图片请求头注入报错）
- 修复 APPX 版本下自动更新尝试失败的问题，改为引导下载安装包手动覆盖安装

### 其他

- 本项目为基于 lx-music-desktop 的第三方魔改版，已更名为 LX-N Music
